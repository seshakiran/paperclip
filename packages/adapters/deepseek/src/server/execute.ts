...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (!apiKey) throw new Error("deepseek adapter missing apiKey");
  
  const baseUrl = asString(config.baseUrl, "https://api.deepseek.com");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`DeepSeek API failed with status ${res.status}: ${errorText}`);
    }
    
    const data = await res.json() as Record<string, unknown>;
    
    let summary = "DeepSeek API call completed";
    if (data.choices && Array.isArray(data.choices) && data.choices.length > 0) {
      const content = (data.choices[0] as Record<string, unknown>).message as Record<string, unknown>;
      summary = (content?.content as string)?.substring(0, 200) ?? "DeepSeek API call completed";
    }
    
    return { exitCode: 0, signal: null, timedOut: false, summary };
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
      return { exitCode: 1, signal: null, timedOut: true, summary: "DeepSeek request timed out" };
    }
    throw error;
  } finally {
    clearTimeout(timer);
