...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (!apiKey) throw new Error("zai adapter missing apiKey");
  
  const baseUrl = asString(config.baseUrl, "https://api.z.ai/v1");
  const model = asString(config.model, "qwen2.5-72b-instruct");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Z.ai API failed with status ${res.status}: ${errorText}`);
    }
    
    const data = await res.json() as Record<string, unknown>;
    
    let summary = "Z.ai API call completed";
    if (data.choices && Array.isArray(data.choices) && data.choices.length > 0) {
      const content = (data.choices[0] as Record<string, unknown>).message as Record<string, unknown>;
      summary = (content?.content as string)?.substring(0, 200) ?? "Z.ai API call completed";
    }
    
    return { exitCode: 0, signal: null, timedOut: false, summary };
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
      return { exitCode: 1, signal: null, timedOut: true, summary: "Z.ai request timed out" };
    }
    throw error;
  } finally {
    clearTimeout(timer);
