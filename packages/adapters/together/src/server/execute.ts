...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (!apiKey) throw new Error("together adapter missing apiKey");
  
  const baseUrl = asString(config.baseUrl, "https://api.together.ai/v1");
  const model = asString(config.model, "meta-llama/Llama-3.3-70B-Instruct");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Together AI API failed with status ${res.status}: ${errorText}`);
    }
    
    const data = await res.json() as Record<string, unknown>;
    
    let summary = "Together AI API call completed";
    if (data.choices && Array.isArray(data.choices) && data.choices.length > 0) {
      const content = (data.choices[0] as Record<string, unknown>).message as Record<string, unknown>;
      summary = (content?.content as string)?.substring(0, 200) ?? "Together AI API call completed";
    }
    
    return { exitCode: 0, signal: null, timedOut: false, summary };
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
      return { exitCode: 1, signal: null, timedOut: true, summary: "Together AI request timed out" };
    }
    throw error;
  } finally {
    clearTimeout(timer);
